...
def test_ios_endpoints():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"❌ Server Status Test Failed: {response.status_code}")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"❌ iOS Compatibility Check Failed: {response.status_code}")
    except Exception as e:
        print(f"❌ iOS Compatibility Check Error: {e}")
    ...
    try:
        ...
        if response.status_code == 200:
            print("✅ iOS Help Page Test:")
            print(f"   Status Code: {response.status_code}")
            print(f"   Content Length: {len(response.text)} bytes")
            
            # Check if it contains iOS-specific content
            if "iOS Safari" in response.text:
                print("   ✅ Contains iOS Safari help content")
            if "QR Code" in response.text:
                print("   ✅ Contains QR code generation")
        else:
            print(f"❌ iOS Help Page Failed: {response.status_code}")
    except Exception as e:
        print(f"❌ iOS Help Page Error: {e}")
