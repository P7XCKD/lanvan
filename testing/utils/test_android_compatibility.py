...
def test_android_environment():
    ...
    print("[BOT] Testing Android/Termux Environment")
    print("=" * 40)
    
    # Check for Termux
    is_termux = os.path.exists('/data/data/com.termux')
    print(f"Termux detected: {'[OK] Yes' if is_termux else '[ERR] No'}")
...
def test_network_detection():
    ...
    print("[NET] Testing Network Detection")
    print("=" * 30)
...
def test_qr_support():
    ...
    print("[MOBILE] Testing QR Code Support")
    print("=" * 25)
    ...
    try:
        ...
        qr.make(fit=True)
        print("[OK] Basic QR generation works")
        
    except ImportError:
        print("[ERR] qrcode library not available")
    except Exception as e:
        print(f"[WARN] QR generation error: {e}")
    
    # Test PIL/Pillow
    try:
        from PIL import Image
        print("[OK] PIL/Pillow available for image QR codes")
    except ImportError:
        print("[WARN] PIL/Pillow not available (text QR codes will be used)")
    
    print()

def test_websocket_support():
    """Test WebSocket library support"""
    print("[CONN] Testing WebSocket Support")
    print("=" * 25)
    
    # Test websockets library
    try:
        import websockets
        print("[OK] websockets library available")
    except ImportError:
        print("[ERR] websockets library not available")
    ...
    try:
        ...
        try:
            import uvloop
            print("[OK] uvloop (performance enhancement) available")
        except ImportError:
            print("[WARN] uvloop not available (install uvicorn[standard] for better performance)")
            
    except ImportError:
        print("[ERR] uvicorn not available")
    
    print()

def test_dependencies():
    """Test all required dependencies"""
    print("[PKG] Testing Dependencies")
    print("=" * 20)
    ...
    for package in required_packages:
        try:
            ...
            print(f"[ERR] {package} - Install with: pip install {package}")
    
    print()

def test_file_permissions():
    """Test file system permissions"""
    print("[DIR] Testing File Permissions")
    print("=" * 25)
    ...
    try:
        ...
        test_file.unlink()
        print("[OK] Current directory writable")
    except Exception as e:
        print(f"[ERR] Current directory not writable: {e}")
    ...
    try:
        ...
        home_test.unlink()
        print("[OK] Home directory writable")
    except Exception as e:
        print(f"[ERR] Home directory not writable: {e}")
        ...
        for path in storage_paths:
            if path.exists():
                try:
                    ...
                    print(f"[WARN] {path} not writable: {e}")
...
def main():
    ...
    print("=" * 15)
    if is_android:
        print("[OK] Running on Android/Termux")
        print("[TIP] Check individual test results above")
        print(" See docs/ANDROID_TROUBLESHOOTING.md for solutions")
    else:
        print("ℹ Not running on Android/Termux")
        print("   This test is designed for Android environments")
    
    print()
    print("[START] If all tests pass, you can run: python run.py")
