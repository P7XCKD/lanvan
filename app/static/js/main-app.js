...
if (typeof show_clipboard_only !== 'undefined' && show_clipboard_only) {
  ...
  function connectClipboardWS() {
    ...
    try {
      ...
      ws.onclose = async () => {
        ...
        // Check if server is actually shutting down or offline
        try {
          ...
          if (data.shutdown) {
            handleServerShutdown(data.message || 'Server is shutting down gracefully');
            return;
          }

          // Server is still alive, reconnect
          const retryDelay = window.isiOSSafari ?
            Math.min(5000, 1000 * wsConnectAttempts) : 1000;
  ...
  function connectRegularClipboardWS() {
    ...
    clipboardWS.onclose = async () => {
      window.log.warn('Clipboard WebSocket disconnected, checking server status...');
      ...
      try {
        ...
        // Server is still alive (just a transient WS drop), reconnect
...
function addToUploadQueue(files) {
  ...
  console.log(` addToUploadQueue: Adding ${files.length} file(s) to upload queue`);
  window.log.upload(`Adding ${files.length} file(s) to upload queue`);

  for (let file of files) {
    const uploadId = ++uploadIdCounter;
    console.log(` Creating upload item for: ${file.name} (ID: ${uploadId})`);
    const uploadItem = createUploadItem(file, uploadId);
    uploadQueue.push(uploadItem);
    console.log(` Upload queue now has ${uploadQueue.length} items`);
...
function startServerStatusMonitoring() {
  // [CLEAN] HTTP Polling timer removed to preserve CPU.
  // Shutdown state is now handled reactively via WebSocket disconnects.
  console.log('[STATUS] Reactive WebSocket-based status monitoring initialized');
}
...
async function uploadFilesChunked(files, isAESEnabled) {
  ...
  //  Guest device large file warning
  if (isGuestDevice && totalSize > LANVAN_CONFIG.GUEST_MEMORY_LIMIT) {
    const fileSizeGB = (totalSize / 1024 / 1024 / 1024).toFixed(1);
    console.warn(` Large file upload (${fileSizeGB}GB) detected on guest device. This may cause memory issues.`);
    showToast(` Large file (${fileSizeGB}GB) on guest device may cause issues. Consider using regular upload or a smaller file.`, 5000);
  }
      ...
      while (currentPosition < fileSize) {
        ...
        try {
          ...
          if (isGuestDevice && chunk.size > LANVAN_CONFIG.CHUNK_SIZES.GUEST_MAX) {
            console.warn(` Large chunk detected on guest device (${(chunk.size / 1024 / 1024).toFixed(1)}MB), this may cause issues`);
          }
        } catch (chunkError) {
...
========================================`);
      ...
      if (imagePromises.length > 0) {
        showToast(` Adding ${imagePromises.length} images to ZIP...`, 2000);
