        ...
        btnSubmit.setOnClickListener {
            ...
            if (includeDiagnostics) {
                ...
                val reportContent = StringBuilder().apply {
                    ...
                    appendLine()
                    appendLine("=== SERVER ===")
                    appendLine("State: ${if (currentState == ServerState.RUNNING) "RUNNING" else "STOPPED"}")
                    appendLine("Protocol: ${if (useHttps) "HTTPS" else "HTTP"}")
                    appendLine("Port: ${ServerService.currentPort}")
                    appendLine()
                    appendLine("=== NETWORK ===")
                    appendLine("Availability: ${if (getLocalIpAddress() != "127.0.0.1") "AVAILABLE" else "UNAVAILABLE"}")
                    appendLine("LAN IP: ${getLocalIpAddress()}")
                    appendLine()
                    appendLine("=== MDNS ===")
                    appendLine("Status: ${if (currentState == ServerState.RUNNING) "ACTIVE" else "INACTIVE"}")
                    appendLine()
                    appendLine("=== SECURITY ===")
                    appendLine("HTTP Dangerous File Protection: ${if (blockHttp) "ON" else "OFF"}")
                    appendLine("HTTPS Dangerous File Protection: ON (Enforced)")
                    appendLine()
                    appendLine("=== BACKGROUND ===")
                    appendLine("Battery Optimization: ${if (isExempted) "Allowed" else "Restricted"}")
                    appendLine()
                    appendLine("=== STORAGE ===")
                    ...
                    appendLine()
                    appendLine("=== SUMMARY ===")
                    appendLine("Server: ${if (currentState == ServerState.RUNNING) "ONLINE" else "OFFLINE"}")
                    appendLine("Network: CONNECTED (${getLocalIpAddress()})")
                    appendLine("Storage: OK (${txtStorageUsage.text})")
                    appendLine("Errors: ${if (ServerService.lastErrorLog.isBlank()) "NONE" else "LOGGED"}")
                }.toString()
